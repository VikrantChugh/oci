...
def insert_availability_zone_details_into_database(zone_list):
    ...
    try:
        ...
        for zones in zone_list:
            insert_query = """
                INSERT INTO cmdb_ci_availability_zone(Fault_domain,Fault_domain_id,Name,Object_id,Account_id,Datacenter,State) 
                values(%s,%s,%s,%s,%s,%s,%s);
            """
            try:
                cursor.execute(insert_query,(zones['Fault_domain'],zones['Fault_domain_id'],zones['Name'],zones['Object_id'],zones['Account_id'],zones['Datacenter'],zones['State']))
