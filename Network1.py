...
def insert_network_detail_into_db(network_list):
    ...
    try:
        ...
        for iteam in network_list:
            insert_query = """
                INSERT INTO cmdb_ci_network(Name,State,Object_Id,Cidr,Domain_Name,AccoundID,Datacenter) 
                values(%s,%s,%s,%s,%s,%s,%s);
            """
            try:
                cursor.execute(insert_query,(iteam['display_name'],iteam['State'],iteam['id'],iteam['cidr_block'],iteam['domain_name'],iteam['account_id'],iteam['Datacenter']))
